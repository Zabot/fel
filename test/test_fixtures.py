def assert_branching_repo(repo, prefix=""):
    assert [4, 3, 2, 1, 0] == [
        int(c.summary) for c in repo.iter_commits(prefix + "branch1")
    ]
    assert [12, 11, 8, 7, 6, 5, 2, 1, 0] == [
        int(c.summary) for c in repo.iter_commits(prefix + "branch2")
    ]
    assert [10, 9, 8, 7, 6, 5, 2, 1, 0] == [
        int(c.summary) for c in repo.iter_commits(prefix + "branch3")
    ]
    assert [14, 13, 6, 5, 2, 1, 0] == [
        int(c.summary) for c in repo.iter_commits(prefix + "master")
    ]
